...
class App extends Component {
  state = {
    tabActive: tabsList[0].tabId,
    mainImage: imagesList[0].imageUrl,
    Score: 0,
    scoreBoardId: imagesList[0].id,
    time: 60,
    winBoard: true,
  }
  ...
  componentDidMount() {
    ...
    this.timerID = setInterval(this.timeCall, 1000)
  }

  updateTabId = activeId => {
    this.setState({tabActive: activeId})
  }

  getFilterImages = () => {
    const {tabActive} = this.state
    const filterImages = imagesList.filter(
      eachImage => eachImage.category === tabActive,
    )
    return filterImages
  }
  ...
  timeOverOrGameOver = () => {
    this.setState({winBoard: false})
    clearInterval(this.timerID)
  }

  getShuffledEmojiList = id => {
    const {scoreBoardId, time} = this.state
    if (id === scoreBoardId && time !== 0) {
      this.setState(prevScore => ({Score: prevScore.Score + 1}))
      const randomImage = Math.random() * imagesList.length
      this.setState({mainImage: imagesList[parseInt(randomImage)].imageUrl})
      const newScoreBoardId = imagesList[parseInt(randomImage)].id
      this.setState({scoreBoardId: newScoreBoardId})
    } else {
      this.timeOverOrGameOver()
    }
  }
  ...
  timeCall = () => {
    ...
    if (time > 1) {
      this.setState(prevTime => ({time: prevTime.time - 1}))
    } else {
      this.setState(prevTime => ({time: prevTime.time - 1}))
      this.timeOverOrGameOver()
    }
  ...
  render() {
    const {tabActive, mainImage, Score, time, winBoard} = this.state
    const filteredListOfImages = this.getFilterImages()
      ...
      <div className="mainPageContainer">
          ...
          <ul className="unorderListNAv">
            ...
            <li className="scoreAndTimeContainer">
              <div className="timerCard">
                ...
                />
                <p className="timeHeading">&nbsp; {time} sec</p>
              </div>
            </li>
          </ul>
        </nav>
        {winBoard ? (
          <div className="DetailedCardContainer">
            <div className="showingImageCard">
              <img src={mainImage} alt="match" className="MainImage" />
            </div>
            <div className="slidesContainer">
              <ul className="slidesName">
                {tabsList.map(eachTab => (
                  <li key={eachTab.tabId} className="tabButton">
                    <button
                      type="button"
                      className={`slideName ${
                        tabActive === eachTab.tabId ? 'activeTabClassName' : ''
                      }`}
                      key={eachTab.tabId}
                      onClick={() => this.updateTabId(eachTab.tabId)}
                    >
                      {eachTab.displayText}
                    </button>
                  </li>
            ...
            <div>
              <ul className="thumbnailContainer">
                {filteredListOfImages.map(eachImage => (
                  <li className="imageItem" key={eachImage.id}>
                    <button
                      type="button"
                      className="thumButton"
                      onClick={() => this.getShuffledEmojiList(eachImage.id)}
                    >
                      <img
          ...
          <div className="winBoardContainer">
